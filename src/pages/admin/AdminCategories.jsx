...
export default function AdminCategories() {
  ...
  function openEdit(c) { setEditing(c.id); setForm({ name: c.name, slug: c.slug, description: c.description || '', icon: c.icon || '', sort_order: c.sort_order, is_active: c.is_active }); setShowForm(true) }
  const set = key => e => { const v = e.target.type === 'checkbox' ? e.target.checked : e.target.value; setForm(f => ({ ...f, [key]: v, ...(key === 'name' && !editing ? { slug: slugify(v) } : {}) })) }

  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.name) { toast.error('Category name is required'); return }
    setSaving(true)
    const payload = { name: form.name, slug: form.slug || slugify(form.name), description: form.description, icon: form.icon, sort_order: Number(form.sort_order), is_active: form.is_active }
    const { error } = editing
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-gray-50">
            {cats.length === 0 && <div className="py-12 text-center text-gray-400">No categories yet.</div>}
            {cats.map(c => (
              <div key={c.id} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">/{c.slug} Order: {c.sort_order}</p>
                </div>
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => openEdit(c)} className="p-1.5 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Pencil size={15} /></button>
                  <button onClick={() => handleDelete(c.id, c.name)} className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={15} /></button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input value={form.name} onChange={set('name')} required placeholder="e.g. Phones & Tablets" className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">URL Slug *</label>
                <input value={form.slug} onChange={set('slug')} required placeholder="phones-tablets" className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Icon (click to select)</label>
                <div className="flex flex-wrap gap-2">
                  {ICONS.map(ic => (
                    <button key={ic} type="button" onClick={() => setForm(f => ({ ...f, icon: ic }))} className={`w-9 h-9 text-xl rounded-lg border-2 transition-all ${form.icon === ic ? 'border-blue-500 bg-blue-50' : 'border-transparent hover:border-gray-200'}`}>{ic}</button>
                  ))}
                </div>
